class HomesController < ApplicationController
  def index
    ...
    @charges_last_month = Charge.where("date >= ? AND date <= ? AND user_id = ?", Time.current.last_month.beginning_of_month, Time.current.last_month.end_of_month, current_user.id)
